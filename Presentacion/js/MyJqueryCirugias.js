...
function ConsultasDeHoyXCirugia()
{
		$.ajax({
			url:'Procesar.php?accion=CargarPacienteCir',
			type:'GET',
			cache:false,
			success:function(res)
			{
				$("#MainRespuestaCirugia").html(res);
			},
			error:function()
			{
				$("#MainRespuestaCirugia").html("error al cargar");
			}
		});
		$("#MenuDoctor").html("");
...
function DiagnosticarCir(codigo,codigo1)
{
...
//menu para el paciente seleccionado
	...
	$("#MenuDoctor").html("<div class='navbar'> <div class='navbar-inner'><div class='brand'>Menu Paciente</div> <ul class='nav pull-right'><li><a href='#' onclick='FinPaciente()'>Finalizar Consulta</a></li>  <li><a href='#' onclick='DatosAllFiliacion("+codigo1+")'>Datos de Filiación</a></li><li><a href='#' onclick='Anamnesis()'>Anamnesis</a></li><li><a href='#' onclick='ExamenFisico2()'>Exámen Físico</a></li><li><a href='#' onclick='Examenfisico()'>Exámenes</a></li><li><a href='#' onclick='DiagnosticoFenix()'>Diagnóstico</a></li> <li><a href='#' onclick='datosvademecun()'>Medicamentos</a></li><!-- <li><a href='#' onclick='Tratamiento2()'>Tratamiento</a></li>--><li class='dropdown'><a href='#' class='dropdown-toggle' data-toggle='dropdown'>Certificados</a><ul class='dropdown-menu'><li><a href='#' onclick='CertificadoAsistencia()' ><span class='icon-file'></span>&nbsp;Asistencia</a></li><li><a href='#' onclick='EnfermedaReposo()'><span class='icon-file'></span>&nbsp;Enfermedad y reposo</a></li><li><a href='#' onclick='CertificadoCuidado()'><span class='icon-file'></span>&nbsp;Cuidado</a></li></li><li><a href='#' onclick='CertificadoCirugia()'><span class='icon-file'></span>&nbsp;Cirugía</a></li><li><a href='#' onclick='CertificadoSalud()'><span class='icon-file'></span>&nbsp;Salud</a></li><li><a href='#' onclick='CertificadoSaludVacunacion()'><span class='icon-file'></span>&nbsp;Salud y vacunación</a></li>    <li><a href='#' onclick='ConsentimientoInfo()'><span class='icon-file'></span>&nbsp;Consentimiento Informado</a></li></ul></ul> </div> </div>");
...
//tabla para llenar el diagnostico con los farmacos	
	...
	$("#LoadaDataNow").html("<center><table class='table-bordered table-striped table-hover table-condensed'><tr><th>Motivo de consulta</th><th>Enefermedad actual</th><th>Examen Fisico</th><th>Examenes</th></tr><tr><td id='MotivoConsultaLoad'></td><td id='EnfermedadAcutalLoad'></td><td id='ExamenFiscoActualLoad'></td><td id='ExamensLoad'></td></tr></table></center>");
...
{
				$.ajax({
					url:'Procesar.php?accion=AsignarTurnoXCirugia&Paciente12='+$('#txtCodpac').val()+'&fechaProx12='+$("#txtFechaProx").val()+'&hora12='+$("#cmb_horas").val(),
					type:'GET',
					cache:false,
					success:function(res)
					{
...
function BuscarHistoriaCirugia()
{
	$( "#HistoriaClinica" ).dialog({
		...
		/*$("#bntBuscaCedulaPac").button();
		$("#bntBuscaCedulaPac").click(function()
		{
			
			$( "#RespuesraHistoria" ).html("<object type='text/html' data='../Reportes/HistoriaCirugia.php?id="+$("#txtBuscarCedula").val()+"'></object>");					
		}); */
}
...
function DiagnosticoFenix()
{
		...
		$( "#HeaderDiagFenix" ).html("<table><tr><!--<td>Buscar Enfermedad por: </td><td><select><option>---Seleccione---</option><option>Descripción</option><option>Código</option></select></td>--><td><center>Buscar: <input type='text' id='txtBucarcie' onkeyup='buscarcie2()'/></center></td></tr></table>");
			...
			$.ajax({
				url:'Procesar.php?accion=VerCieCir',
				type:'GET',
				cache:false,
				success:function(res)
				{
...
{
	$.ajax({
				url:'Procesar.php?accion=VerCie3Cir&Descripcion33='+$("#txtBucarcie").val(),
				type:'GET',
				cache:false,
				success:function(res)
				{
...
function AsignarCie(codigo)
{
	...
	$.ajax({
				url:'Procesar.php?accion=AsigCieCirugia&Cie='+codigo,
				type:'GET',
				cache:false,
				success:function(res)
				{
...
function VerCitas()
{
		...
		$( "#NewCitas" ).dialog( "open" );
			$.ajax({
				url:'Procesar.php?accion=VerCitasAllHoyCir',
				type:'GET',
				cache:false,
				success:function(res)
				{
...
function LugarCirugia(lugar)
{
	
	
	$.ajax({
		url:'Procesar.php?accion=agendarLugarCir&LugCirugia='+lugar,
		type:'GET',
		cache:false,
		success:function(res)
				{
		...
		$.ajax({
			...
			success:function(res)
			{
				...
				setTimeout(function(){
					...
					//LugarCirugia($("#txtLugar").val());
					$(".txtLugar").each(function() {
						if($(this).val()!="")
						{
						
							LugarCirugia($(this).val());
						}
                    });
				...
				setTimeout(function(){ReloadCitas()},1000);
				//setTimeout(function(){ConsultasDeHoyXCirugia()},500);
				setTimeout(function()
				{
...
function AsignarPacienteEmeregencia(codigo)
{
		$.ajax({
			...
			{
				$("#RespuestaAsignacion").html(res);
				setTimeout(function(){ReloadCitas(),ConsultasDeHoyXCirugia()});
...
{
	$("#RespuestaAsignacion").html("Buscar Fecha: <input type='text' id='txtFechaAgendDoct'/> <input type='button' class='btn btn-success' id='bntOk321' value='Agendar' />");
	...
	//ajax 
			...
			$.ajax({
				url:'Procesar.php?accion=HorarioConsutaCir&FechaPoxima='+$('#txtFechaAgendDoct').val()+'&CodigoForma='+2,
				type:'GET',
				cache:false,
				success:function(res)
				{
...
function LugarCirugiaFecha(lugar2)
{
	
	
	$.ajax({
		url:'Procesar.php?accion=agendarLugarCir2&LugCirugia2='+lugar2,
		type:'GET',
		cache:false,
		success:function(res)
				{
...
function GenerarTurnoPacienteGeneralXDoctor()
{
			...
			$.ajax({
				...
				success:function(res)
				{
					...
					LugarCirugiaFecha($("#txtLugar2").val());
					},1000);
					setTimeout(function(){ReloadCitas()},1000);
				setTimeout(function()
				{
	...
	$( "#estadopago" ).dialog({
		...
		$("#btn_pago").click(function(){
			if($("#st_pago").val()!=""){
				...
				{
					$("#formpago").html(res);
					setTimeout(function(){ConsultasDeHoyXCirugia();},1000);
				},
				error:function()
				{
...
function IngresoSalidaCir()
{
	var codipaciente=$("#codigoPaciente").val();
	var coditurno=$("#codigoTurno123").val();
	$.ajax({
			url:'Procesar.php?accion=ControCir&codigopaciente='+codipaciente+'&codigoturno='+coditurno,
			type:'GET',
			cache:false,
			success:function(res)
			{
...
{
	alert($("#txtHonorarios").val());
	var codigocir=$("#codigoTurno123").val();
	$.ajax({url:'Procesar.php?accion=salidacirugia&codigocir='+codigocir+'&fecha='+$("#txtfechaalta").val()+'&TipoCiru='+$("#TCirugia").val()+'&honorario='+$("#txtHonorarios").val(),
			type:'GET',
			cache:false,
			success:function(res)
			{
			...
			error:function()
			{
				$("#respuesta").html("Error Al Cargar");
			}
		});
	}
