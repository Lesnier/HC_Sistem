jQuery(function(){
	...
	function drawDiente(svg, parentGroup, diente){
		...
		$.each([caraCentral, caraIzquierda, caraDerecha, caraInferior, caraSuperior, caraCompleto], function(index, value){
	    	value.click(function(){
				...
				$("#DiagNosTOdontolo").append("P"+diente.id+cara+" - "+tratamiento.nombre+";");
	...
	var borrar;
	var texto;
	texto=$("#DiagNosTOdontolo").html();
		self.quitarTratamiento = function(tratamiento){
			texto=$("#DiagNosTOdontolo").html();
			borrar="P"+tratamiento.diente.id+tratamiento.cara+" - "+tratamiento.tratamiento.nombre+";";
			var res =	texto.replace(borrar,"");
			$("#DiagNosTOdontolo").html(res);
