...
function FancyTable()
{
	...
	$expe=new AnamnesisCdu;
	$id=$_GET['id'];
	//$id=$id+1;
	$med=$expe->Consultar("SELECT medico_cduanam FROM tbl_cduanamnesis WHERE id_cduanam='$id';");
	$idpac=$expe->Consultar("SELECT id_pac FROM tbl_cduanamnesis WHERE id_cduanam='$id';;");
	$ced=$expe->Consultar("SELECT cedula_pac FROM tbl_paciente WHERE id_pac='$idpac';");
	$nombpac=$expe->Consultar("SELECT nombresCom_pac FROM tbl_paciente WHERE id_pac='$idpac';");
	$planesmed=$expe->Consultar("SELECT planesdte_cduanam FROM tbl_cduanamnesis WHERE id_cduanam='$id';");
	// Colores, ancho de línea y fuente en negrita
	...
	$this->SetXY(2.5,11);
	$this->SetFont('Arial','b',8);
	$this->MultiCell(17,0.5,utf8_decode("$planesmed  "),0,1,'l',true);
	
		$idmed=$expe->Consultar("SELECT id_med FROM tbl_cduanamnesis WHERE id_cduanam='$id';");
		$img=$expe->Consultar("SELECT url_usu FROM tbl_usuario WHERE id_usu='$idmed';");
		if($img!=""){
