...
function FancyTable()
{
	...
	$id=$_GET['id'];
	//$id=$id+1;
	$med=$solic->Consultar("SELECT med_intsoli FROM tbl_solicitudinterconsulta WHERE id_intsoli='$id'");
	$idpac=$solic->Consultar("SELECT id_pac FROM tbl_solicitudinterconsulta WHERE id_intsoli='$id'");
	$ced=$solic->Consultar("SELECT cedula_pac FROM tbl_paciente WHERE id_pac='$idpac'");
	$nombpac=$solic->Consultar("SELECT nombresCom_pac FROM tbl_paciente WHERE id_pac='$idpac'");
	$planterare=$solic->Consultar("SELECT plante_intsoli FROM tbl_solicitudinterconsulta WHERE id_intsoli='$id'");
	...
	$this->SetXY(2.5,11);
	$this->SetFont('Arial','b',8);
	$this->MultiCell(17,0.5,utf8_decode("$planterare  "),0,1,'l',true);
	
		$idmed=$solic->Consultar("SELECT id_med FROM tbl_solicitudinterconsulta WHERE id_intsoli='$id'");
		$img=$solic->Consultar("SELECT url_usu FROM tbl_usuario WHERE id_usu='$idmed'");
