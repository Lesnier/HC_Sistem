...
function FancyTable()
{
	...
	$id=$_GET['id'];
	//$id=$id+1;
	$med=$epi->Consultar("SELECT medico_epi FROM tbl_epicrisis WHERE id_epi='$id'");
	$idpac=$epi->Consultar("SELECT id_user FROM tbl_epicrisis WHERE id_epi='$id'");
	$ced=$epi->Consultar("SELECT cedula_pac FROM tbl_paciente WHERE id_pac='$idpac'");
	$nombpac=$epi->Consultar("SELECT nombresCom_pac FROM tbl_paciente WHERE id_pac='$idpac'");
	$resument=$epi->Consultar("SELECT rdtypt_epi FROM tbl_epicrisis WHERE id_epi='$id'");
	...
	$this->SetXY(2.5,11);
	$this->SetFont('Arial','b',8);
	$this->MultiCell(17,0.5,utf8_decode("$resument  "),0,1,'l',true);
		
		$idmed=$epi->Consultar("SELECT id_med FROM tbl_epicrisis WHERE id_epi='$id'");
		$img=$epi->Consultar("SELECT url_usu FROM tbl_usuario WHERE id_usu='$idmed'");
		if($img!=""){
