...
class PDF extends FPDF
{
	...
	{
		$aux=new Consulta;
		$datos=$aux->Consultar_HistoriaCir($cod); 
		return $datos;	
	}
	...
	function BasicTable($header,$data)
	{
		...
		foreach($data as $row)
		{
			...
			$this->MultiCell(400,6,utf8_decode("Fecha de Ingreso: ". $row['FechaIngreso']),2,'L');
			$this->MultiCell(400,6,utf8_decode("Fecha de Alta: ". $row['FechaAlta']),2,'L');
			$this->MultiCell(400,6,utf8_decode("Tipo Cirugía Realizada: ".$row['TipoCirugia']),2,'L');
			$this->MultiCell(400,6,utf8_decode("Lugar de la Cirugía: ".$row['LugarCirugia']),2,'L');
			$this->MultiCell(400,6,utf8_decode("Estado del Pago: ".$row['Pago']),2,'L');
			$this->MultiCell(400,6,utf8_decode("Honorarios: ".$row['Honorario']),2,'L');
	...
	function Header()
	{
		...
		$this->MultiCell(100,14,"Historia Clinica \t\t\t\t\t Unimed",0,'L');
...
$cod= $_GET['id'];
$pdf=new PDF('L','mm','A4');
$header=array('HistoriaCirugia','FechaAlta','TipoCirugia','LugarCirugia','Pago','Honorario');
