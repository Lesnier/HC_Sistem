...
class PDF extends FPDF
{
	...
	function BasicTable($header,$data)
	{
		...
		foreach($data as $row)
		{
				...
				$diag=$aux->Consultar("SELECT desc_diag FROM tbl_cie WHERE id_diag='$row[id_cie]'");
				$ciefghj=$aux->Consultar("SELECT cod_diag FROM tbl_cie WHERE id_diag='$row[id_cie]'");
				
			$this->MultiCell(400,6,utf8_decode("Fecha de consulta: ". $row['fechaProx_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Diagnostico: ". $ciefghj."    ".$diag),1,'L');
			$this->MultiCell(400,6,utf8_decode("Medicamentos: ".$row['vademecun_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Nombre comercial: ".$row['nomcomercial_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Cantida: ".$row['cantidad_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Dosis: ".$row['dosis_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Via de administracion: ".$row['viaAdmin_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Frecuencia: ".$row['frecuencia_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Numero: ".$row['numduracion_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Duracion: ".$row['duracion_cons']),1,'L');
			$this->MultiCell(400,6,utf8_decode("Tratamiento: ".$row['tratamiento_cons']),1,'L');
	...
	function Header()
	{
		...
		//$da=new Consultas;
		//$cedula= $_GET['id'];
		//$nombrePac=$da->Consultar("SELECT nombresCom_pac FROM tbl_paciente WHERE cedula_pac='$cedula'");		
		//$EdadPac=$da->Consultar("SELECT fechaN_pac FROM tbl_paciente WHERE cedula_pac='$cedula'");				
		//$DirecPac=$da->Consultar("SELECT direccion_pac FROM tbl_paciente WHERE cedula_pac='$cedula'");						
		...
		$this->MultiCell(100,14,"Receta \t\t\t\t\t ",0,'L');
